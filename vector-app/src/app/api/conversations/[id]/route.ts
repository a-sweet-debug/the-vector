...
export async function GET(
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT title, description, summary_markdown, architecture_markdown, marketing_markdown, finance_markdown FROM projects WHERE id = $1',
      [numericId]
    );
    ...
    // Reconstruct the chat history from the saved data so the Board Room can render it!
    const messages = [
